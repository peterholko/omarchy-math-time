"""Verify the parent/root password through this plugin's own PAM service."""
import ctypes
...
def parent_password_ok(password):
    ...
    result = pam.pam_start(b"peterholko-math-time", b"root", ctypes.byref(conv), ctypes.byref(handle))
    if result != 0:
        return False
    try:
